  ...
  methods:{
      ...
      getinfo(){
          ...
          httpFetch.get('http://life.microger.com:8086/api/share/deviceInfo/loadDeviceInfo?deviceNumber='+this.deviceNumber,{
                ...
                if(res.data.timeDeviceChargingWayDOList.length!=0){
                    //按时间
                    this.timetype = [];
                    for(var i=0; i<res.data.timeDeviceChargingWayDOList.length;i++){
                        var obj = {}
                        obj.type = res.data.timeDeviceChargingWayDOList[i].meteringUnitName
                        this.timetype.push(obj)
                    }
                    this.price = res.data.timeDeviceChargingWayDOList[0].meteringUnitPrice
                    this.mintime = res.data.timeDeviceChargingWayDOList[0].meteringUnitSmall
                    this.maxtime = res.data.timeDeviceChargingWayDOList[0].meteringUnitMax
                    // this.timetype = res.data.timeDeviceChargingWayDOList[0].meteringUnitName
                    localStorage.setItem('amountMoney',this.num1*this.price)
                    localStorage.setItem('lease_number',this.num1)
                    localStorage.setItem('price',res.data.timeDeviceChargingWayDOList[0].meteringUnitPrice)
                    localStorage.setItem('mintime',res.data.timeDeviceChargingWayDOList[0].meteringUnitSmall)
                    localStorage.setItem('maxtime',res.data.timeDeviceChargingWayDOList[0].meteringUnitMax)
                    // localStorage.setItem('timetype',res.data.timeDeviceChargingWayDOList[0].meteringUnitName)
                    localStorage.setItem('deviceNumber',this.deviceNumber);
                    localStorage.setItem('id',res.data.timeDeviceChargingWayDOList[0].id);
                    localStorage.setItem('chargingWayType',res.data.timeDeviceChargingWayDOList[0].chargingWayType);
                    this.data = res.data.timeDeviceChargingWayDOList
